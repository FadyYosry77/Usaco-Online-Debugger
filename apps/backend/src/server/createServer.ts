...
export async function createServer(context: AppContext) {
  ...
  app.post("/session/sync-code", async (request, reply) => {
    try {
      const body = syncCodeSchema.parse(request.body) as SyncCodeRequest;
      const snapshot = await context.sessionService.syncCode(
        body.sourceCode,
        body.sourceFileName,
        body.pageUrl
      );
  ...
  app.post("/build", async (request, reply) => {
    try {
      const body = buildSchema.parse(request.body ?? {}) as BuildRequest;
      ...
      const result = await context.buildService.build(
        body.sourceFileName ?? session.sourceFileName,
        sourceCode,
        settings,
        body.profile ?? "usaco"
      );
  ...
  app.post("/run", async (request, reply) => {
    try {
      const body = runSchema.parse(request.body) as RunRequest;
      const session = await context.sessionService.getSnapshot();
      if (!session.sourceCode.trim()) {
        return reply.code(400).send(noSyncedSourceBuildResult());
      }
      const settings = await context.settingsService.getSettings();
      const buildResult = await context.buildService.build(
  ...
  app.post("/debug/start", async (request, reply) => {
    try {
      const body = debugStartSchema.parse(request.body ?? {}) as DebugStartRequest;
      const session = await context.sessionService.getSnapshot();
      if (!session.sourceCode.trim()) {
        return reply.code(400).send(noSyncedSourceBuildResult());
      }
  ...
  app.post("/debug/run-to-line", async (request, reply) => {
    try {
      const body = runToLineSchema.parse(request.body) as RunToLineRequest;
  ...
  app.post("/debug/toggle-breakpoint", async (request, reply) => {
    try {
      const body = breakpointSchema.parse(request.body) as ToggleBreakpointRequest;
  ...
  app.post("/debug/evaluate", async (request, reply) => {
    try {
      const body = evaluateSchema.parse(request.body) as EvaluateRequest;
  ...
  app.post("/debug/watches", async (request, reply) => {
    try {
      const body = watchesSchema.parse(request.body) as SaveWatchesRequest;
  ...
  app.post("/debug/memory", async (request, reply) => {
    try {
      const body = memorySchema.parse(request.body) as MemoryReadRequest;
  ...
  app.post("/testcases", async (request, reply) => {
    try {
      const body = testcaseCreateSchema.parse(request.body) as TestCaseCreateRequest;
  ...
  app.put("/testcases/:id", async (request, reply) => {
    try {
      const body = testcaseUpdateSchema.parse(request.body) as TestCaseUpdateRequest;
      const testcase = await context.testCaseService.update((request.params as { id: string }).id, body);
      if (!testcase) {
  ...
  app.delete("/testcases/:id", async (request) => {
    const removed = await context.testCaseService.remove((request.params as { id: string }).id);
    return { removed };
  });
